...
create table KHOAHOC (
    ...
    NGAYBD date,
    NGAYKT date,
    constraint kh_ck check (NGAYKT>NGAYBD)
    );
...
CREATE TABLE DIEM (
    MAMH REFERENCES MONHOC(MAMH),
    MAHV REFERENCES HOCVIEN(MAHV),
...
-- thêm ko duoc vi date cua du lieu khhac voi date cua table

--Câu 4
INSERT INTO LOP VALUES ('L004','LL002','L?p 4',10,'K001');
-- khong them duoc vi siso < 12

--câu 5
DELETE FROM KHOAHOC WHERE MAKH = 'K001';
select * from KHOAHOC
-- xoa khoa học K001 được


--câu 6
DELETE FROM KHOAHOC WHERE MAKH = 'K002';
select * from KHOAHOC
-- xoa khoa học K002 được
...
DROP TABLE HOCVIEN_NAM; --xóa được
