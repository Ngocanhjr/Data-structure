...
int Bellman_Ford(Graph* pG, int s) {
        ...
        for (int u = 1; u <= pG->n; u++) {
            ...
            for (int v = 1; v <= pG->n; v++)
                ...
                if (pG->W[u][v] != NO_EDGE)
                    ...
                    if (d[v] > (d[u] + pG->W[u][v]) && d[u] != oo)
                        // Cập nhật d[v] = d[u] + edgeWeight(u, v)
                        d[v] = d[u] + pG->W[u][v];
        }
    }

    // Lặp thêm 1 lần nữa để xem có thể cập nhật d[v] nào nữa không
    // Duyệt qua tất cả các đỉnh u
    for (int u = 1; u <= pG->n; u++) {
        // Xét các đỉnh kề v của u và cập nhật d[v]
        for (int v = 1; v <= pG->n; v++)
    ...
    return 1;  // G không chứa chu trình âm
}
...
int main() {
    ...
    if (!Bellman_Ford(&G, 1))
        cout << "G has a negative cycle." << endl;
    else {
        cout << "G does not contain any negative cycle." << endl;
