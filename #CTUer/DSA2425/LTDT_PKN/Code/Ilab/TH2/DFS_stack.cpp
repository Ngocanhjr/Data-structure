...
void DFS(Graph* pG, int s){
	//1. Declare stack, initialize empty stack
	Stack S;
	make_null_stack(&S);
	
	//2. Push s into S, start traversing from vertex s
	push(&S,s);
	
	//3. loop
	while(!is_empty_stack(&S)){
		//3.a retrieve top element from stack
		int u = top(&S);
		pop(&S);
		...
		if(!visited[u]){
			printf("%d \n", u);
			visited[u] = 1;
			// Consider adjacent edges of u, push into stack
			for(int v = pG->n; v > 0; v--){
				if(pG->A[u][v] != 0){
					push(&S, v);
				}
			}
